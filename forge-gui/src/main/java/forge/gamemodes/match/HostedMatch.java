...
public class HostedMatch {
    ...
    public void startGame() {
        ...
        for (int iPlayer = 0; iPlayer < players.size(); iPlayer++) {
            ...
            p.updateSleeve();
            // The mat comes from the lobby slot, so every player sees the one its owner picked.
            // Game modes that don't go through the lobby fall back to this machine's preference.
            p.getLobbyPlayer().setMatKey(rp.getPlayer().getMatKey());
